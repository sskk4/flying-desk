...
import { Clock, Plus, X, ArrowLeft, Upload } from "react-feather"; 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const deskResponse = await axios.post(
        `http://localhost:8081/api/v1/building/${buildingId}/desk`,
        formDataToSend,
        {
          headers: {
      ...
      if (deskResponse.data && deskResponse.data.id) {
        const deskId = deskResponse.data.id;
      ...
      setMessage("Desk and availability added successfully!");
      setFormData({
        desk: "",
        equipment: "",
        description: "",
      ...
      setFiles([]);
      navigate(`/owner/desks`);
    } catch (err) {
      setError("Error creating desk or availability. Try again.");
      console.error(err);
    } finally {
      setLoading(false);
    }
      ...
      <div className="manage-ads-container">
        <div className="form-container card-shadow">
          <form onSubmit={handleSubmit}>
            ...
            <h2 className="form-title">Create an ad for your desk</h2>
